...
macro_rules! impl_try_as_imdbid_str {
    ($type:ty) => {
        impl TryAsImdbid for $type {
            ...
            fn digits(&self, pad: usize) -> Result<String, Error> {
                self.valid().map(|_| match &self[0..2] {
                    "tt" => format_digits!((self.split_at(2).1), pad),
                    _ => format_digits!(self, pad),
                })
            }
...
macro_rules! impl_try_as_imdbid_num {
    ($type:ty) => {
        impl TryAsImdbid for $type {
            ...
            fn digits(&self, pad: usize) -> Result<String, Error> {
                self.valid().map(|_| format_digits!(self, pad))
            }
        }
    };
}
...
macro_rules! impl_try_as_imdbid_path {
    ($type:ty) => {
        impl TryAsImdbid for $type {
            fn valid(&self) -> Result<(), Error> {
                match self
                ...
                {
                    Some(_) => Ok(()),
                    None => Err(Error::NoValidAncestors),
                }
            }
            fn digits(&self, pad: usize) -> Result<String, Error> {
                match self
...
impl_try_as_imdbid_path!(std::path::Path);
impl_try_as_imdbid_path!(std::path::PathBuf);
